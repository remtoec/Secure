...
zh: {
  ...
  "landing.body": "依附系統的工作很簡單：人腦隨時在確認，親近的人此刻找不找得到、靠不靠得住。答案穩定，人就放得開，去冒險、去專心做自己的事；答案飄忽，注意力就被抽走，用來查看訊息、猜語氣、盤算下一步。Levine 常用小孩做例子：照顧者在房間，小孩自顧自玩玩具；照顧者一走開，玩具立刻失去吸引力。大人的機制相同，只是對象換了人。<br><br>研究用兩條軸描述你在一段關係中的位置：<strong>焦慮</strong>，你多常擔心對方不夠在乎你；<strong>迴避</strong>，你依賴對方時有多自在。你對媽媽、對伴侶、對好友，可以落在三個不同的位置；成人依附與童年的關聯亦不足一成，位置會隨經驗移動。所以這個工具逐段關係測量，並讓你隔一段時間重測，把移動畫在同一張圖上。",
  "landing.start": "開始測驗 →",
  ...
  "collab.bandHigh": "多數安全指標都亮綠燈。這段關係經得起投入，繼續用 CARRP 的方式維持它。",
  ...
  "styles": {
    secure:  {name:"安全型",
      ...
      tip:"這種穩定靠日常維持：對方需要時在場、有回應、說到做到（書中簡稱 CARRP）。Levine 有一條吵架規則值得試：一次只准一個人發脾氣，另一個負責穩住場面。你穩，身邊的人也會慢慢穩。"},
    anxious: {name:"焦慮型",
      ...
      tip:"你的敏感是真的：關係出問題，你通常最早知道。消耗你的是後面那段：試探、發脾氣或冷戰，事後又後悔（Levine 稱之為抗議行為）。下次跳過試探，直接講需要，然後看對方的反應。可靠的人會用行動讓你放心；令你長期猜來猜去的人，那份不安本身就是資訊。"},
    avoidant:{name:"迴避型",
      desc:"焦慮低、迴避高。你習慣自己搞定：事情自己處理，情緒自己消化。這個人想再靠近，你會覺得空間被壓縮，自然退開。",
