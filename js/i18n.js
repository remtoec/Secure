...
zh: {
  ...
  "landing.body": "依附研究數十年來的一個重要發現：依附風格並非一個固定的「人格標籤」——你面對媽媽時可以是安全型，面對伴侶時卻可能偏向焦慮型。Amir Levine 在《Secure》一書中對此有深入淺出的討論。本工具採用研究中廣泛使用的兩個維度，透過 9 條簡單題目，為你和每一位重要的人計算<strong>焦慮</strong>與<strong>迴避</strong>兩個分數，繪成一張屬於你的「依附地形圖」；隔一段時間重測，更可以看到每段關係隨時間移動的軌跡。",
  "landing.start": "開始測驗 →",
  ...
  "landing.howBody": "<strong>逐段關係測</strong>：伴侶、媽媽、爸爸、好友、同事⋯⋯每位對象一種顏色，全部畫在同一張圖上。<br><strong>隔期重測</strong>：關係會變，座標也會移動——地形圖會為你連起每次紀錄的軌跡。<br><strong>關係檢核</strong>：15 條「這段關係值得投入嗎？」檢核表，很適合讀書會討論。",
  ...
  "result.styleIsGeneral": "你目前的整體依附傾向屬於：",
  "result.snapshotNote": "這是此刻、這段關係的自我觀察，不是固定標籤。",
  "result.snapshotNoteGeneral": "這是此刻的自我觀察，不是固定標籤。",
  "result.anxLabel": "焦慮分數（第 7–9 題平均，1–7）",
  "result.avdLabel": "迴避分數（第 1–6 題平均，1–7）",
  "result.tipTitle": "給你的建議",
  ...
  "collab.bandLow": "這段關係在多個安全指標上偏低。書中建議：先嘗試用 CARRP 的方式修補互動，若持續沒有改善，可考慮「調低音量、降低優先次序」。",
  ...
  "styles": {
    secure:  {name:"安全型",
      ...
      descGeneral:"低焦慮、低迴避。整體而言，你能自在地與人親近、表達需要，也不太擔心被拋棄。",
      tip:"繼續保持 CARRP——一致、可得、有回應、可靠、可預測。安全感是會傳染的：你的穩定也會令身邊的人更安全。"},
    anxious: {name:"焦慮型",
      desc:"高焦慮、低迴避。你渴望親近這個人，但常擔心對方不夠在乎你、可能離開你，情緒容易隨對方的一舉一動起伏。",
      descGeneral:"高焦慮、低迴避。整體而言，你渴望與人親近，但常擔心對方不夠在乎你，情緒容易隨關係的狀態起伏。",
      tip:"焦慮型的敏銳可以是一種很有用的覺察力，但也容易被不安放大。留意「抗議行為—後悔」循環；直接、平靜地說出需要，並觀察對方是否 CARRP——值得的人會用行動讓你安心。"},
    avoidant:{name:"迴避型",
      desc:"低焦慮、高迴避。你重視獨立和空間，不習慣向這個人打開心扉，親密貼近反而令你不自在。",
      descGeneral:"低焦慮、高迴避。整體而言，你重視獨立和空間，不習慣向人打開心扉，太過親近反而令你不自在。",
      tip:"需要距離並非罪過，但長期迴避會令對方被觸發、關係惡性循環。嘗試小步分享感受，主動給予微小而穩定的回應（正面的 SIMIs）。"},
    fearful: {name:"恐懼迴避型",
      desc:"高焦慮、高迴避。你既渴望親密又害怕受傷，於是時而靠近、時而抽離，關係中經常感到拉扯。",
      descGeneral:"高焦慮、高迴避。整體而言，你既渴望親密又害怕受傷，時而靠近、時而抽離。",
...
en: {
  ...
  "landing.body": "A key finding from decades of attachment research: attachment style is not a fixed personality label — you can be secure with your mum yet lean anxious with your partner. Dr. Amir Levine discusses this in depth in <em>Secure</em>. This tool uses the two dimensions widely used in the research — <strong>anxiety</strong> and <strong>avoidance</strong> — to score 9 short questions about one specific relationship and plot it on your personal “attachment topography” map. Retake it over time and see how each relationship shifts.",
  ...
  "result.styleIs": "Right now, this relationship appears to be: ",
  "result.styleIsGeneral": "Right now, your overall pattern appears to be: ",
  "result.snapshotNote": "This is a snapshot of this relationship, not a fixed label.",
  "result.snapshotNoteGeneral": "This is a snapshot of where you are now, not a fixed label.",
  ...
  "collab.bandLow": "Several security indicators are low. The book's suggestion: try repairing the interaction in a CARRP way first; if nothing improves, consider turning down the volume and lowering this relationship's priority.",
  ...
  "styles": {
    secure:  {name:"Secure",
      ...
      descGeneral:"Low anxiety, low avoidance. Overall, you're comfortable getting close and expressing needs, without much fear of being left.",
    ...
    anxious: {name:"Anxious",
      desc:"High anxiety, low avoidance. You crave closeness with this person but often worry they don't care enough or might leave, and your emotions may rise and fall with their signals.",
      descGeneral:"High anxiety, low avoidance. Overall, you crave closeness but often worry that others don't care enough, and your mood tracks how your relationships are going.",
      tip:"Anxious sensitivity can be a useful form of awareness, but it can also be amplified by insecurity. Watch the protest-behaviour → regret cycle; state your needs directly and calmly, and check whether this person is CARRP — the right people reassure you through action."},
    avoidant:{name:"Avoidant",
      ...
      descGeneral:"Low anxiety, high avoidance. Overall, you value independence and space, rarely open up, and too much closeness feels uncomfortable.",
    ...
    fearful: {name:"Fearful-avoidant",
      ...
      descGeneral:"High anxiety and high avoidance. Overall, you long for intimacy yet fear getting hurt, swinging between approaching and withdrawing.",
